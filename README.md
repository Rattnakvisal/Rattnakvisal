<!-- ========================================================= -->
<!-- GitHub Profile README - Chun Rattnakvisal                 -->
<!-- ========================================================= -->

<!-- Hero Banner -->
<p align="center">
  <img
    src="https://capsule-render.vercel.app/api?type=waving&height=280&text=Chun%20Rattnakvisal&fontAlign=50&fontAlignY=38&desc=Full-Stack%20Web%20Developer%20%7C%20Laravel%20%7C%20ASP.NET%20Core%20%7C%20React%20%7C%20Flutter&descAlign=50&descAlignY=60&color=0:0f172a,50:14532d,100:22c55e&fontColor=ffffff&animation=fadeIn"
    alt="Chun Rattnakvisal Banner"
  />
</p>

<!-- Typing Animation -->
<p align="center">
  <img
    src="https://readme-typing-svg.herokuapp.com?size=25&duration=2500&pause=800&color=22C55E&center=true&vCenter=true&width=950&lines=👋+Hi%2C+I'm+Chun+Rattnakvisal;💻+Full-Stack+Web+Developer;🚀+Laravel+%7C+ASP.NET+Core+%7C+React+%7C+Flutter;☕+Java+%7C+PostgreSQL+%7C+MySQL;🛒+E-commerce+System+Builder;🔐+Security+%26+Performance+Focused"
    alt="Typing Animation"
  />
</p>

<!-- Social Links -->
<p align="center">
  <a href="https://github.com/Rattnakvisal">
    <img src="https://img.shields.io/badge/GitHub-Rattnakvisal-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="mailto:rattnakvisalchun@gmail.com">
    <img src="https://img.shields.io/badge/Email-rattnakvisalchun%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://www.linkedin.com/in/chun-rattnakvisal-3a24a0361/">
    <img src="https://img.shields.io/badge/LinkedIn-Chun%20Rattnakvisal-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>

---

## 👨‍💻 About Me

Hi, I'm **Chun Rattnakvisal**, a passionate **Full-Stack Web Developer** who enjoys building modern, secure, and scalable web applications.

I focus on turning ideas into real-world digital products using **clean code**, **beautiful UI design**, and **reliable backend systems**.

---

## 🚀 Tech Stack

### 🌐 Frontend
<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,tailwind,bootstrap" alt="Frontend Skills" />
</p>

### ⚙️ Backend
<p align="left">
  <img src="https://skillicons.dev/icons?i=php,laravel,dotnet,cs,java,nodejs" alt="Backend Skills" />
</p>


### 🗄️ Database
<p align="left">
  <img src="https://skillicons.dev/icons?i=mysql,postgresql,sqlite" alt="Database Skills" />
</p>

### 🧰 Tools & Platforms
<p align="left">
  <img src="https://skillicons.dev/icons?i=git,github,postman,vscode,figma,npm,vite" alt="Tools" />
</p>

---

## 💼 What I Build

<table>
  <tr>
    <td>✅ Full-stack web applications</td>
    <td>✅ E-commerce systems</td>
  </tr>
  <tr>
    <td>✅ Admin dashboards</td>
    <td>✅ Authentication systems</td>
  </tr>
  <tr>
    <td>✅ RESTful APIs</td>
    <td>✅ Payment gateway integrations</td>
  </tr>
  <tr>
    <td>✅ Database-driven applications</td>
    <td>✅ Responsive modern UI</td>
  </tr>
</table>

---

## 🛠️ Featured Skills

<table>
  <tr>
    <td width="50%">
      <h3>🌐 Frontend Development</h3>
      <p>
        Building responsive, clean, and user-friendly interfaces using
        <b>HTML</b>, <b>CSS</b>, <b>JavaScript</b>, <b>React</b>,
        <b>Tailwind CSS</b>, and <b>Bootstrap</b>.
      </p>
    </td>
    <td width="50%">
      <h3>⚙️ Backend Development</h3>
      <p>
        Creating secure and scalable backend systems using
        <b>Laravel</b>, <b>ASP.NET Core</b>,
        <b>REST APIs</b>, and database integrations.
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>🗄️ Database Management</h3>
      <p>
        Working with <b>MySQL</b>, <b>PostgreSQL</b>, and <b>SQLite</b>
        for storing, organizing, and managing application data.
      </p>
    </td>
  </tr>
</table>

---

## 📊 GitHub Stats
<p align="center">
  <img
    src="https://github-readme-activity-graph.vercel.app/graph?username=Rattnakvisal&theme=tokyo-night&hide_border=true"
    alt="GitHub Activity Graph"
  />
</p>

---

## 🎯 Current Focus

- 🌱 Learning advanced backend architecture  
- 📱 Improving Flutter mobile development skills  
- ☕ Practicing Java for backend and OOP  
- 🐘 Working more with PostgreSQL  
- 💳 Learning payment gateway integration  
- 🔐 Improving web security and authentication  
- 📚 Writing cleaner and more scalable code  

---

## 🏆 Goals 2026

- 🚀 Build strong portfolio projects  
- 💻 Master Laravel + React ecosystem  
- 🐘 Use PostgreSQL in real projects  
- 🔐 Improve system design and security  
- 🌍 Contribute to open-source projects  
- 📈 Become a stronger full-stack developer  

---

## 📫 Contact Me

<p align="center">
  <a href="https://github.com/Rattnakvisal">
    <img src="https://img.shields.io/badge/GitHub-Rattnakvisal-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="mailto:rattnakvisalchun@gmail.com">
    <img src="https://img.shields.io/badge/Email-rattnakvisalchun%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://www.linkedin.com/in/chun-rattnakvisal-3a24a0361/">
    <img src="https://img.shields.io/badge/LinkedIn-Chun%20Rattnakvisal-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>

---

<p align="center">
  <b>✨ Let's build something amazing together 🚀</b>
</p>
